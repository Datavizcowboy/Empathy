...
d3.json("../DATA/opportunities.json", function (error, data) {
    ...
    mount.selectAll('polygon').data(data.data).enter()
        ...
        .style("fill", function (d, i) { return d3.rgb(0 - i * 6, 250 - i * 2, 250 + i * 2); })
        .style("opacity", 1);
    ...
    mount2.selectAll('polygon').data(data.data).enter()
        ...
        .attr("points", function (d, i) {
            var pitchbar = 1
            var a = -0 + (i) * 70 + "," + 420
            var b = -0 + (i) * 70 + (d.click_count * .4) + "," + (420 - (d.click_count * .8))
            var c = -0 + (i) * 70 + (d.click_count * .8) + "," + 420
            return (a + " " + b + " " + c);
        })
        .style("fill","#333")
        .style("opacity", .8);
    ...
    mountlineclick.selectAll('line').data(data.data).enter()
        .append('line')
        .attr("x1", function (d, i) { return -0 + (i) * 70 + (d.click_count * .4); })
        .attr("y1", function (d) { return (412 - (d.click_count * .8)); })
        .attr("x2", function (d, i) { return -0 + (i) * 70 + (d.click_count * .4); })
        .attr("y2", function (d) { return (395 - (d.click_count * .8)); })
        .attr("stroke-width", 1)
        .style("opacity", 1)
        .attr("stroke", "#333");
    ...
    mounttextclick.selectAll('text').data(data.data).enter()
        .append("text")
        .attr("x", function (d, i) { return -2 + (i) * 70 + (d.click_count * .4); })
        .attr("y", function (d) { return (390 - (d.click_count * .8)); })
    ...
    mounttextclick2.selectAll('text').data(data.data).enter()
        .append("text")
        .attr("x", function (d, i) { return -2 + (i) * 70 + (d.click_count * .4); })
        .attr("y", function (d) { return (380 - (d.click_count * .8)); })
        .text(function (d, i) { return d.terms; })
        .attr("font-family", "Gill Sans, Century Gothic, sans-serif")
        .attr("font-size", label_size)
        .style("opacity", .8)
